...
bool Board::existSolutions() const{
    ...
    for(auto pointOne: this->points){
        for(auto pointTwo: this->points){
            if(pointOne.getX()==pointTwo.getX()&&pointOne.getY()==pointTwo.getY())continue;
...
bool Board::canUnion(const Point& pointOne, const Point& pointTwo) const{
    for(auto segment: this->segments){
        if(doesInteracts(segment, pointOne, pointTwo)){
            return false;
        }
    }
    return true;
...
bool Board::doesInteracts(const Segment& segment, const Point& pointOne, const Point& pointTwo)const{
    int orientationOne = computerOrientaion(segment.getA(),segment.getB(),pointOne);
    int orientationTwo = computerOrientaion(segment.getA(),segment.getB(),pointTwo);
    int orientationThree = computerOrientaion(pointOne, pointTwo, segment.getA());
    int orientationFour = computerOrientaion(pointOne,pointTwo, segment.getB());
...
int Board::computerOrientaion(const Point& A, const Point& B, const Point& C) const {
