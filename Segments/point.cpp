...
if(abs(x-this->x)>DEFAULT_RADIUS+100) return false;
if(abs(y-this->y)>DEFAULT_RADIUS+100) return false;

int distanceFromCenterSquared=(x-this->x)*(x-this->x)+(y-this->y)*(y-this->y);

return distanceFromCenterSquared<=DEFAULT_RADIUS*DEFAULT_RADIUS+100;
}
